    ...
    for uid, regs in reg_map.items():
        ...
        for i in range(0, len(payload_bytes), 2):
            if i + 1 < len(payload_bytes):
                high = payload_bytes[i]
                low = payload_bytes[i+1]
                decrypted_pixels.add((low, high)) # X=low, Y=high
                
    if not decrypted_pixels:
        print("No pixels found.")
        return
        
    max_x = max(p[0] for p in decrypted_pixels)
    max_y = max(p[1] for p in decrypted_pixels)
    
    with open('decrypted_flag_image.txt', 'w', encoding='utf-8') as f:
        for y in range(max_y + 1):
            line = ""
            for x in range(max_x + 1):
                if (x, y) in decrypted_pixels:
                    line += "██"
                else:
                    line += "  "
            f.write(line + "\n")
