...
def extract_ascii():
    ...
    for uid, regs in sorted(reg_map.items()):
        ...
        for addr in sorted_addrs:
            ...
            high = (val >> 8) & 0xFF
            low = val & 0xFF
            text_be += chr(high) if 32 <= high <= 126 else '.'
            text_be += chr(low) if 32 <= low <= 126 else '.'
            
            text_le += chr(low) if 32 <= low <= 126 else '.'
            text_le += chr(high) if 32 <= high <= 126 else '.'
            
        print(f"Unit {uid} BE: {text_be}")
        print(f"Unit {uid} LE: {text_le}\n")
