import struct

def render_font():
    with open('industrial_meltdown.pcap', 'rb') as f: data = f.read()
    magic = struct.unpack('<I', data[0:4])[0]
    endian = '<' if magic == 0xA1B2C3D4 else '>'
    offset = 24
    
    SCADA = "192.168.100.20"
    
    # Grid of 40 characters, each 16x16
    grid = [[" " for _ in range(16 * 40)] for _ in range(16)]
    
    while offset < len(data) - 16:
        incl_len = struct.unpack(endian + 'I', data[offset+8:offset+12])[0]
        pkt = data[offset+16:offset+16+incl_len]
        offset += 16 + incl_len
        
        if len(pkt) > 54 and struct.unpack('>H', pkt[12:14])[0] == 0x0800:
            ip = pkt[14:]
            if len(ip) > 20 and ip[9] == 6:
                dst_ip = '.'.join(str(b) for b in ip[16:20])
                ihl = (ip[0] & 0xF) * 4
                tcp = ip[ihl:]
                if len(tcp) >= 20:
                    src_port = struct.unpack('>H', tcp[0:2])[0]
                    doff = ((tcp[12] >> 4) & 0xF) * 4
                    payload = tcp[doff:]
                    
                    if dst_ip == SCADA and src_port == 502 and len(payload) >= 8:
                        fc = payload[7]
                        if fc == 3 and len(payload) >= 9:
                            bc = payload[8]
                            if len(payload) >= 9 + bc:
                                reg_bytes = payload[9:9+bc]
                                for i in range(0, bc, 2):
                                    if i + 1 < bc:
                                        val = struct.unpack('>H', reg_bytes[i:i+2])[0]
                                        char_idx = (val >> 8) & 0xFF
                                        pixel_idx = val & 0xFF
                                        
                                        if char_idx < 40:
                                            # pixel_idx from 0 to 255
                                            # Assuming row-major order: y = pixel_idx // 16, x = pixel_idx % 16
                                            # Or maybe column-major: x = pixel_idx // 16, y = pixel_idx % 16
                                            # Let's try row-major first
                                            py = pixel_idx // 16
                                            px = pixel_idx % 16
                                            
                                            global_x = char_idx * 16 + px
                                            if py < 16 and global_x < 16 * 40:
                                                grid[py][global_x] = "█"
                                                
    with open('flag_font.txt', 'w', encoding='utf-8') as f:
        for row in grid:
            line = "".join(row)
            f.write(line + "\n")
            print(line)

if __name__ == "__main__":
    render_font()
